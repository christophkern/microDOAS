...
//printf("NA\r\n");
  ...
  else if(nmea_cmd_type == NMEA_GPRMC)
  {
    ...
    {
        Speed = gprmc.speed * 0.51444;
        Course = gprmc.course;
    }
    sprintf(dstr, "%06ld", Date);
    dstr2[0] = dstr[0];
    dstr2[1] = dstr[1];
    Day = atoi(dstr2);
    dstr2[0] = dstr[2];
...
void* start_gps(void *arg/*int argc,char *argv[]*/)
{
  //---------------------------------------------------------------------------------------
  ...
  // open serial port
  char GPSport[12];
  sprintf(GPSport, "/dev/tty%s", cfg.GPS.TTYPort.c_str());
  if (cfg.GPS.TTYPort[0] == 'S')
      GPSport[10] = '\0';
  fd = open(GPSport, O_RDWR | O_NOCTTY );
  if(fd<0) 
  {
  ...
  if (cfg.GPS.BaudRate == 4800)
    newtio.c_cflag = BAUDRATE4800 | CRTSCTS | CS8 | CLOCAL | CREAD;
  else if (cfg.GPS.BaudRate == 9600)
    newtio.c_cflag = BAUDRATE9600 | CRTSCTS | CS8 | CLOCAL | CREAD;
  else if (cfg.GPS.BaudRate == 19200)
    newtio.c_cflag = BAUDRATE19200 | CRTSCTS | CS8 | CLOCAL | CREAD;
  else if (cfg.GPS.BaudRate == 38400)
    newtio.c_cflag = BAUDRATE38400 | CRTSCTS | CS8 | CLOCAL | CREAD;
  else if (cfg.GPS.BaudRate == 57600)
    newtio.c_cflag = BAUDRATE57600 | CRTSCTS | CS8 | CLOCAL | CREAD;
  else if (cfg.GPS.BaudRate == 115200)
    newtio.c_cflag = BAUDRATE115200 | CRTSCTS | CS8 | CLOCAL | CREAD;
  else
  {
    printf("ERROR! You have chosen an unsupported GPS baud rate!!\n");
    printf("SUPPORTED BAUD RATES: 4800, 9600, 19200, 38400, 57600, 115200\n");
    printf("Defaulting to 4800 baud...");
    newtio.c_cflag = BAUDRATE4800 | CRTSCTS | CS8 | CLOCAL | CREAD;
  }
